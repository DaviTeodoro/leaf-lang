...
describe(evaluate, () => {
  ...
  it('should be able to evaluate a random number', () => {
    const ast = { type: 'Identifier', name: 'random' };
    console.log(evaluate(ast));
    expect(evaluate(ast)).toBeGreaterThan(0);
    expect(evaluate(ast)).toBeLessThan(1);
  });
});
